...
describe("Deploy contracts, mint character, progress through campaign", function () {
  ...
  it("Generate turn 4, update state correctly", async () => {
    ...
    const invBefore = await CastleCampaignContract.getInventory(0);
    console.log(invBefore);

    await expect(CastleCampaignContract.connect(user1).generateTurn(0))
      .to.emit(CastleCampaignContract, "TurnStarted")
      .withArgs(CastleCampaignContract.address, 0, playerTurn, 2);

    expect(await CastleCampaignContract.turnInProgress(0)).to.equal(true);

    const invAfter = await CastleCampaignContract.getInventory(0);
    console.log(invAfter);
    expect(invAfter[0].name).to.equal("Dragonslayer's Ice Lance");
  });

  //test is psuedorandom so I know only 1 mob
  it("Execute turn 4, state updates properly", async () => {
    await CastleCampaignContract.connect(user1).endExploreLoot(0);
  ...
  it("Attacks Dragon Boss --- reads back values", async () => {
    ...
    await CastleCampaignContract.connect(user1).attackWithItem(0, 0, 0);
    const bossAfterItem = await CastleCampaignContract.combatTurnToMobs(
      0,
      0,
      5,
      0
    );
    const userCampaignStatusAfterItem =
      await CastleCampaignContract.playerStatus(0, 0);
    console.log(
      "Turn: ",
      ethers.utils.formatUnits(playerTurn, "wei"),
      " Mob 1 Health: ",
      bossAfterItem.health
    );
    console.log("Player Health :", userCampaignStatusAfterItem.health);
    ...
    while (!turnFinished) {
      ...
      console.log("Player Health :", userCampaignStatusAfter.health);
      bossAfter.health == 0 ? (turnFinished = true) : (turnFinished = false);
    }
